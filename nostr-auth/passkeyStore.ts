...
/** App name shown in the OS passkey/Face ID prompt (WebAuthn RP name). */
const appName = (): string => getNostrAuthConfig().appName || 'Nostr App';
...
export async function saveWithPasskey(nsec: string, displayName: string): Promise<void> {
  ...
  const credName = `${displayName} · ${appName()}`;
