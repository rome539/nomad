...
export async function cmdGo(z: ZoneDO, session: Session, dir: string): Promise<void> {
  ...
  if (session.seizedBy) {
    ...
    // The typed break reads weak-gripped the same way the tick's does (zone.ts)
    // — the grip is the animal's, not the command's.
    } else if (chance(SEIZE_BREAK_ODDS * (grip.traits?.includes("weak-gripped") ? MOB_WEAKGRIP_MULT : 1)
      + (z.wearsTrait(session, "slick") ? SLICK_BREAK_BONUS : 0))) {
      session.seizedBy = undefined;
...
export async function cmdGet(z: ZoneDO, session: Session, arg: string, fromDive = false): Promise<void> {
  ...
  if (tookRoamRock || z.world!.groundSpawns.some((g) => g.item_id === itemId && g.room_id === session.roomId && g.regrows)) {
    ...
    if (!stillHere && !alreadyRegrowing) {
      ...
      const dice = (tmpl.slot !== "" || DICE_REGROW.has(itemId)) && !RELIABLE_GEAR.has(itemId);
      z.regrow.push({
        itemId,
