...
// THE CUSTODIAL GOOGLE PATH IS GONE (2026-08-07).
//
// handleGoogle sealed the player's SECRET KEY with KEY_ENC_SECRET and kept it in
// google_accounts, handing it back on any device that signed in with Google.
// That made the dungeon a key custodian: a server-side compromise, or anyone who
// could mint a Google token for a sub, took the wanderer's identity outright.
//
// It was superseded by the Drive vault, where the nsec is encrypted CLIENT-side
// under a PIN (PBKDF2-SHA256) and stored in the user's OWN Drive — we never hold
// the key or the ciphertext. The endpoint had already been unrouted (index.ts
// says as much) and the client had no reference left to it, so this was 60 lines
// of dead code whose only feature was holding secrets. Dead code that can only
// hurt you is worth deleting rather than keeping warm.
//
// The google_accounts TABLE is deliberately left in place: dropping it would
// destroy the only copy of the key for anyone who linked under the old scheme
// and never migrated. It is simply no longer read or written.
